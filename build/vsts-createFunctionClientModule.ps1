...
$parentModule = 'PSAzureMigrationAdvisor'
if (-not $ModuleName) { $ModuleName = 'PSAzureMigrationAdvisor.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\PSAzureMigrationAdvisor'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\PSAzureMigrationAdvisor.psd1"
$prereqHash = @{
