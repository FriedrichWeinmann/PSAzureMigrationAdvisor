...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'PSAzureMigrationAdvisor.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'PSAzureMigrationAdvisor.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'PSAzureMigrationAdvisor.Client.ProtectedToken'
